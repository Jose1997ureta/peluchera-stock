...
export function Table<T>({
  ...
  // A flexible column (no explicit width, e.g. a "description" column) absorbs the
  // container's leftover space on its own. When every column is explicitly sized
  // instead, there's nothing to grow — shrink-wrap the table to the columns' own
  // widths so it doesn't stretch and leave a blank gap after the last column.
  const allColumnsSized =
    orderedColumns.length > 0 &&
    orderedColumns.every((c) => (widths[c.key] ?? c.width) != null)
  ...
  return (
    <div
      className={cn(
        'overflow-hidden border border-border bg-background text-sm',
        allColumnsSized ? 'w-fit max-w-full' : 'w-full',
        className,
      )}
    >
      <div
