...
/** Kept out of SelectContextValue (deliberately): that context is a reactive,
 * useMemo'd value re-created on state changes, and mixing a ref into it makes
 * the compiler treat every read off it — including plain state like `open` —
 * as a render-time ref access. `contentRef` itself only needs to be read inside
 * `Select` (the outside-click check), which already closes over it directly —
 * so this context only ever needs to carry the plain setter function, never a
 * ref-typed value, and stays untainted. */
interface SelectRefContextValue {
  setContentNode: (node: HTMLDivElement | null) => void;
}

const SelectRefContext = createContext<SelectRefContextValue | null>(null);

function useSelectRefContext(component: string) {
  const ctx = useContext(SelectRefContext);
  if (!ctx) throw new Error(`${component} must be used within <Select>`);
  return ctx;
}
...
export function Select({
  ...
  useEffect(() => {
    ...
    const onPointer = (e: PointerEvent) => {
      ...
      if (!insideRoot && !insideContent) setOpen(false);
    };
  ...
  const refCtx = useMemo<SelectRefContextValue>(
    () => ({ setContentNode }),
    [setContentNode],
  );
  ...
  return (
    <SelectContext.Provider value={ctx}>
      <SelectRefContext.Provider value={refCtx}>
...
export function SelectContent({ className, children }: SelectContentProps) {
  ...
  const refCtx = useSelectRefContext("SelectContent");
  const innerRef = useRef<HTMLDivElement>(null);
  // Stable identity: an inline arrow here would get a new reference every
  // render, so Motion — reading `ref` change as "this is a new element" —
  // would skip the tween and just snap straight to whatever `animate` target
  // that render happens to compute, freezing mid-close the moment any parent
  // state (e.g. the filter's onValueChange) re-renders this component.
  const setContentRef = useCallback(
    (node: HTMLDivElement | null) => refCtx.setContentNode(node),
    [refCtx],
  );
  ...
  // Portaled to <body>, so it's positioned from the viewport (not a relative
  // ancestor) and must track the trigger's rect itself — recomputed on open
  // and kept in sync while scrolling/resizing so it never drifts off it.
  useLayoutEffect(() => {
    if (!open) return;
  ...
  // Portaled anchor coordinates: flush against the trigger (top-full/bottom-full
  // equivalent) computed from its viewport rect, since fixed positioning can't
  // rely on a `relative` ancestor once rendered into <body>. Falls back to the
  // origin before the first open — invisible either way (opacity 0, height 0).
  const a = anchor ?? { top: 0, bottom: 0, left: 0, width: 0 };
  ...
  return createPortal(
    <motion.div
      ...
      style={{
        ...
        left: a.left,
        width: a.width,
        top: isTop ? undefined : a.bottom,
        bottom: isTop ? window.innerHeight - a.top : undefined,
        transformOrigin: isTop ? "bottom" : "top",
        overflow: "hidden",
        pointerEvents: open ? "auto" : "none",
      }}
      className={cn(
        // above CenterMorphModal's z-[100] overlay, since this portals to the
        // same <body> target and DOM order alone wouldn't win the stack.
        "z-[110] rounded-xl border border-border bg-background shadow-lg",
        className,
      )}
    >
      <motion.div
