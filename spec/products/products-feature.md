...
- Columnas (mismas en ambos tabs): contador de fila (`#`, número secuencial según orden/página actual, no persiste ni es un id), imagen, nombre, descripción, precio, stock, acciones. La tabla solo muestra el número de stock, sin ningún badge o anotación de "stock bajo" — esa alerta vive únicamente en la métrica del dashboard (`LOW_STOCK_THRESHOLD`, ver `spec/dashboard/dashboard-feature.md`), no en el listado de productos.
...
- Formulario de producto, en este orden: `nombre`, `descripción`, `precio` (numérico, admite decimales, ej. `1234.50`), `stock` (numérico entero), y por último `imagen` (una sola, con preview más grande que la miniatura de la tabla, antes de "subir"). El estado activo/desactivado **no** se edita desde el formulario — solo desde las acciones de la tabla.
...
## Contratos de datos
- Semilla mock: `src/shared/data/mock-products.json` — array de productos con la forma `{ id, name, description, price, stock, imageUrl, isActive, createdAt, updatedAt }`, igual forma que tendrá la tabla `products` real de `AGENTS.md` sección 3 (en camelCase porque es JSON de cliente, no la fila de Postgres). Todos los productos de la semilla arrancan con `isActive: true`.
...
- Schema de validación: `src/features/products/schemas/product.schema.ts` (Yup) — `name`: requerido; `description`: opcional; `price`: requerido, numérico, > 0; `stock`: requerido, entero, >= 1; `image`: archivo opcional (o `imageUrl` existente en edición), máx. 5 MB, formatos permitidos. `isActive` no forma parte de este schema (no se edita desde el formulario).
...
- El tab Activos muestra solo productos con `isActive: true`; el tab Desactivados muestra solo los `isActive: false`. Ambos con las mismas columnas, incluida la descripción, y el stock siempre como número simple, sin badge.
