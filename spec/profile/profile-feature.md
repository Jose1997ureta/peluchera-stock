...
- La foto de perfil sigue el mismo patrón que la imagen de producto (`spec/products/products-feature.md`): una sola imagen (no galería), hasta 5 MB, formatos `jpg`/`jpeg`/`png`/`webp`, se procesa a `data URL` recién al confirmar el submit de esa sección (mientras tanto solo preview local con `URL.createObjectURL`).
